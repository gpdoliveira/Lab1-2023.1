...
int pontuacao=0,lastpontuacao=0;
int combos=0;
int tabconcluidos=0;
int bombasdisp=0;
int undodisp=0;
...
void calculaSomas(int somacol[5],int somalin[5],struct pecas *vetordepecas,struct tabuleiro matriztab[5][5],int contadorpecas){
    int combo=0;
    for(int i=0;i<5;i++){
        somacol[i]=0;
        somalin[i]=0;
    ...
    for (int i=0;i<5;i++){
        ...
        somacol[0]+=matriztab[i][0].valor;
    }
    if (somacol[0]==10){
        somacol[0]=0;
        for (int i=0;i<5;i++){
            matriztab[i][0].valor=0;
            if (matriztab[i][0].ocupado==1){
                matriztab[i][0].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        colunasApaga(0,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[i][1].valor!=-1)
        somacol[1]+=matriztab[i][1].valor;
    }
    if (somacol[1]==10){
        somacol[1]=0;
        for (int i=0;i<5;i++){
            matriztab[i][1].valor=0;
            if (matriztab[i][1].ocupado==1){
                matriztab[i][1].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        colunasApaga(1,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[i][2].valor!=-1)
        somacol[2]+=matriztab[i][2].valor;
    }
    if (somacol[2]==10){
        somacol[2]=0;
        for (int i=0;i<5;i++){
            matriztab[i][2].valor=0;
            if (matriztab[i][2].ocupado==1){
                matriztab[i][2].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        colunasApaga(2,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[i][3].valor!=-1)
        somacol[3]+=matriztab[i][3].valor;
    }
    if (somacol[3]==10){
        somacol[3]=0;
        for (int i=0;i<5;i++){
            matriztab[i][3].valor=0;
            if (matriztab[i][3].ocupado==1){
                matriztab[i][3].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        colunasApaga(3,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[i][4].valor!=-1)
        somacol[4]+=matriztab[i][4].valor;
    }
    if (somacol[4]==10){
        somacol[4]=0;
        for (int i=0;i<5;i++){
            matriztab[i][4].valor=0;
            if (matriztab[i][4].ocupado==1){
                matriztab[i][4].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        colunasApaga(4,matriztab,vetordepecas,contadorpecas);
        combo++;
    }
    ...
    for (int i=0;i<5;i++){
        ...
        somalin[0]+=matriztab[0][i].valor;
    }
    if (somalin[0]==10){
        somalin[0]=0;
        for (int i=0;i<5;i++){
            matriztab[0][i].valor=0;
            if (matriztab[0][i].ocupado==1){
                matriztab[0][i].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        linhasApaga(0,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[1][i].valor!=-1)
        somalin[1]+=matriztab[1][i].valor;
    }
    if (somalin[1]==10){
        somalin[1]=0;
        for (int i=0;i<5;i++){
            matriztab[1][i].valor=0;
            if (matriztab[1][i].ocupado==1){
                matriztab[1][i].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        linhasApaga(1,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[2][i].valor!=-1)
        somalin[2]+=matriztab[2][i].valor;
    }
    if (somalin[2]==10){
        somalin[2]=0;
        for (int i=0;i<5;i++){
            matriztab[2][i].valor=0;
            if (matriztab[2][i].ocupado==1){
                matriztab[2][i].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        linhasApaga(2,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[3][i].valor!=-1)
        somalin[3]+=matriztab[3][i].valor;
    }
    if (somalin[3]==10){
        somalin[3]=0;
        for (int i=0;i<5;i++){
            matriztab[3][i].valor=0;
            if (matriztab[3][i].ocupado==1){
                matriztab[3][i].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        linhasApaga(3,matriztab,vetordepecas,contadorpecas);
        combo++;
    }

    for (int i=0;i<5;i++){
        if (matriztab[4][i].valor!=-1)
        somalin[4]+=matriztab[4][i].valor;
    }
    if (somalin[4]==10){
        somalin[4]=0;
        for (int i=0;i<5;i++){
            matriztab[4][i].valor=0;
            if (matriztab[4][i].ocupado==1){
                matriztab[4][i].ocupado=0;
                lastpontuacao=pontuacao;
                pontuacao+=1;
            }
        }
        linhasApaga(4,matriztab,vetordepecas,contadorpecas);
        combo++;
    }
...
void fjogarOn(int minutos,int segundos,int *contadorpecas,struct pecas *vetordepecas,ALLEGRO_FONT *font,struct tabuleiro matriztab[5][5],struct slot1 *s1,struct slot2 *s2,struct slot3 *s3,int *overOn,int *jogarOn,int somacol[5],int somalin[5]){
    ...
    al_draw_textf(font, al_map_rgb(255, 255, 255), 810, 690, ALLEGRO_ALIGN_CENTRE, "Giros restantes: %d", combos);
...
void inicializaVPecas(struct pecas *vetordepecas){
    ...
    for(int i=0;i<200;i++){
        for (int a=0;a<2;a++){
            for (int b=0;b<2;b++){
                if (a!=0 && b!=0){
                    if (vetordepecas[i].pecaunit[a][b].valor==0){
                        vetordepecas[i].pecaunit[a][b].valor=-1;
                    }
                }
...
void printaRecord(ALLEGRO_FONT *font){
    ...
    al_draw_textf(font, al_map_rgb(255, 255, 255), 500, 500, ALLEGRO_ALIGN_CENTRE, "Pontuacao: %d Timer: %d:%d",ponts,mins,segs);
}
